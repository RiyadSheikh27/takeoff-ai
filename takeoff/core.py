"""
Material Takeoff Core Logic - AI Processing Module
This is your EXACT code with zero modifications
"""
...
STEEL_WEIGHTS = {
    ...
    'W24X117': 117, 'W24X104': 104, 'W24X94': 94, 'W24X84': 84,
    'W24X76': 76, 'W24X68': 68, 'W24X62': 62, 'W24X55': 55,
    ...
    'W8X21': 21, 'W8X18': 18, 'W8X15': 15, 'W8X13': 13, 'W8X10': 10,
    ...
    'HSS10X10X5/16': 31.84, 'HSS10X10X1/4': 25.82,
    'HSS8X8X5/8': 48.86, 'HSS8X8X1/2': 40.23, 'HSS8X8X3/8': 31.05,
    'HSS8X8X5/16': 26.21, 'HSS8X8X1/4': 21.28,
    'HSS6X6X5/8': 38.47, 'HSS6X6X1/2': 31.76, 'HSS6X6X3/8': 24.28,
    'HSS6X6X5/16': 20.57, 'HSS6X6X1/4': 16.74,
...
def extract_ocr(pdf_path):
    ...
    counts = {'beams': Counter(), 'hss': Counter(), 'channels': Counter(), 'angles': Counter(), 'joists': Counter()}
                    ...
                    for span in line["spans"]:
                        for pattern in [W_BEAM, HSS, CHANNEL, ANGLE, JOIST]:
                            for match in pattern.finditer(span["text"]):
                                positions[normalize(match.group())].append({'page': page_num, 'bbox': span["bbox"]})
    doc.close()
    return {'counts': counts, 'positions': dict(positions)}

# OpenAI Analysis
def analyze_openai(pdf_path, api_key):
    if not api_key: return Counter()
    try:
        ...
        for page_num in range(len(doc)):
            ...
            try:
                ...
                if json_match:
                    for k, v in json.loads(json_match.group()).items():
        ...
        for page_num in range(len(doc)):
            ...
            try:
                resp = model.generate_content([
                    "Count ALL structural steel members in this drawing. Count EVERY instance. Return ONLY JSON: {\"W30X90\": 81, \"W27X84\": 34}",
                    img
                ])
                content = resp.text
                json_match = re.search(r'\{[^{}]*\}', content, re.DOTALL)
...
def reconcile(ocr_data, openai_counts, gemini_counts):
    all_members = set()
    for c in ocr_data['counts'].values(): all_members.update(c.keys())
    ...
    for member in all_members:
        ...
        if ocr_count > 0:
            ...
            if openai_count > 0 and gemini_count > 0:
                final_count, confidence, method = int(round(openai_count * 0.6 + gemini_count * 0.4)), 0.80, "AI Consensus"
            elif openai_count > 0:
...
def create_highlighted_pdf(pdf_path, results, output_path):
    doc = fitz.open(pdf_path)
    colors = {'Beam': (0,0.7,0), 'HSS': (0.9,0.5,0), 'Channel': (0.8,0,0.8), 'Angle': (0,0.8,0.8), 'Joist': (0.8,0.8,0)}
    ...
    for cat, cnt in highlight_counts.most_common(): legend += f"  {cat}: {cnt}\n"

    doc[0].insert_textbox(fitz.Rect(10,10,250,160), legend, fontsize=9, fontname="helv", color=(0,0,0), fill=(1,1,0.9), border_width=1)
    doc.save(output_path)
    doc.close()

# Create Excel
def create_excel(results, output_path, pdf_path):
    rows = [{'Designation': r['designation'], 'Category': r['category'], 'Quantity': r['quantity'],
             ...
             'OCR': r['ocr_count'], 'OpenAI': r['openai_count'], 'Gemini': r['gemini_count']} for r in results]

    df = pd.DataFrame(rows)
    summary = pd.DataFrame({
        'Metric': ['File', 'Total Members', 'Types', 'Weight (lbs)', 'Weight (tons)', 'Date', 'Version'],
        'Value': [os.path.basename(pdf_path), df['Quantity'].sum(), len(df),
                  f"{df['Total Weight (lbs)'].sum():,.0f}", f"{df['Total Weight (lbs)'].sum()/2000:.2f}",
                  datetime.now().strftime('%Y-%m-%d %H:%M'), 'SaaS v1.0']
    })
