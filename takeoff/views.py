"""
Views for Material Takeoff App - FIXED VERSION
Key fixes:
1. Higher DPI for better OCR quality
2. Proper error handling without breaking AI flow
3. Better file handling to prevent corruption
...
def process_pdf(request):
    ...
    CRITICAL FIXES:
    1. Ensure file is fully written before processing
    2. Use absolute paths consistently
    3. Verify file integrity before AI processing
    ...
    try:
        ...
        # Write file in binary mode with proper error handling
        with open(pdf_path, 'wb') as destination:
            for chunk in pdf_file.chunks():
                destination.write(chunk)

        # CRITICAL: Verify file was written successfully
        if not os.path.exists(pdf_path) or os.path.getsize(pdf_path) == 0:
            return JsonResponse({'error': 'File upload failed - file is empty'}, status=500)

        print(f"✅ File saved: {pdf_path} ({os.path.getsize(pdf_path)} bytes)")
        ...
        if not openai_key and not gemini_key:
            print("⚠️ WARNING: No API keys provided. Using OCR only.")

        # =================================================================
        # YOUR AI CODE RUNS HERE - UNCHANGED
        # =================================================================
        ...
        if not results:
            return JsonResponse({
                'error': 'No structural members found in PDF. Please check if the PDF contains readable text.'
            }, status=400)
